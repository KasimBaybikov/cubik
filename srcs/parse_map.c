...
/*   Updated: 2021/03/08 13:54:14 by rvernon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (x < 0 || y < 0 || x >= all->map_w || y >= all->map_h ||
			all->map[y][x] == ' ' || all->map[y][x] == 0)
		error(4, all);
	if (all->map[y][x] != new && all->map[y][x] != ' ' &&
			all->map[y][x] != '1' && all->map[y][x] != 0)
	{
		all->map[y][x] = new;
...
void	make_map(t_all *all, t_list *head, int size)
{
	...
	while (all->map[i])
	{
		all->map_w = ((int)ft_strlen(all->map[i]) > all->map_w) ?
			(int)ft_strlen(all->map[i]) : all->map_w;
		i++;
	}
