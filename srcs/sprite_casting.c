...
/*   Updated: 2021/03/08 14:02:32 by rvernon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sprite_dist(t_sprite *sprr, t_spr *spr, t_plr *plr)
{
	...
	{
		sprr->spr_order[i] = i;
		sprr->spr_dist[i] = ((plr->x - spr[i].x) * (plr->x - spr[i].x) +
				(plr->y - spr[i].y) * (plr->y - spr[i].y));
		i++;
	}
}
...
void	calc_sprite(t_all *all, t_sprite *s, int i)
{
	...
	s->inv_det = 1.0 / (all->plr->plane_x * all->plr->dir_y - all->plr->dir_x *
			all->plr->plane_y);
	s->transform_x = s->inv_det * (all->plr->dir_y * s->spr_x -
			all->plr->dir_x * s->spr_y);
	s->transform_y = s->inv_det * (-all->plr->plane_y *
			s->spr_x + all->plr->plane_x * s->spr_y);
	s->spr_screen_x = (int)((all->win->w / 2) *
...
void	draw_sprite(t_all *all, t_sprite *s, int stripe)
{
	...
	while (y < s->dr_end_y)
	{
		...
		color =
		((int*)all->sprite->img_data)[all->sprite->width * s->tex_y + s->tex_x];
...
void	sprite_casting(t_all *all, t_sprite *s)
{
	...
	while (i < s->num_sprites)
	{
		...
		{
			s->tex_x = (int)((256 * (s->stripe - (-s->spr_w /
						2 + s->spr_screen_x)) * 64 / s->spr_w) / 256);
			if (s->transform_y > 0 && s->stripe > 0 &&
			s->stripe < all->win->w && s->transform_y < all->z_buf[s->stripe])
			{
