...
/*   Updated: 2021/03/08 13:44:35 by rvernon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_w(t_all *all, t_plr *plr)
{
	...
	if (all->map[(int)(all->plr->y)]
			[(int)(all->plr->x + plr->dir_x * step)] != '1')
		all->plr->x += plr->dir_x * plr->move_speed;
	if (all->map[(int)(all->plr->y + plr->dir_y * step)]
			[(int)(all->plr->x)] != '1')
...
void	move_a(t_all *all, t_plr *plr)
{
	...
	step = 0.11;
	if (all->map[(int)plr->y]
			[(int)(plr->x - plr->plane_x * step)] != '1')
		all->plr->x -= plr->plane_x * plr->move_speed;
	if (all->map[(int)(plr->y - plr->plane_y * step)]
			[(int)(plr->x)] != '1')
